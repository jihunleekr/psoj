...
function getObjFilename(srcFilename) {
  ...
  if (ext === "ts") extra = ".js";
  if (ext === "cs") extra = ".exe";
  if (platform() === "win32" && ["c", "cc", "cpp", "go", "rs"].includes(ext)) {
    extra = ".exe";
  }
...
function compile(srcFilename, dir) {
  ...
  switch (ext) {
    ...
    case "cpp":
      ...
      break;
    case "cs":
      result = spawnSync("bash", ["-c", `tee /dev/null | csc -out:${objFilename} ${srcFilename}`], options);
      break;
    case "go":
...
function run(srcFilename, input, dir) {
  ...
  switch (ext) {
    ...
    case "cc":
      ...
      break;
    case "cs":
      if (platform() === "win32") result = spawnSync(`./${objFilename}`, options);
      else result = spawnSync("mono", [`${objFilename}`], options);
      break;
    case "go":
      result = spawnSync(`./${objFilename}`, options);
      break;
